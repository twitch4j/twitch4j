...
public class ChannelBaseEvent extends TwitchBaseEvent {
	 ...
	 */
	public ChannelBaseEvent(Channel channel) {
		super();
		this.channel = channel;
	}

	private User getBot() {
		Optional<OAuthCredential> credentials = getTwitchClient().getCredentialManager().getTwitchCredentialsForIRC();
		if (credentials.isPresent()) {
			return getTwitchClient().getUserEndpoint().getUser(credentials.get()).orElse(null);
		} else return null;
	}

	private boolean isBotModerator() {
		User botUser = getBot();
		return (botUser != null) && getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getModerators().contains(botUser);
	}

	private boolean isBotChannelEditor() {
		User botUser = getBot();
		return (botUser != null) && getTwitchClient().getChannelEndpoint(channel.getId()).getEditors().contains(botUser);
	}
	...
	public void sendMessage(String message) {
		getTwitchClient().getMessageInterface().sendMessage(getChannel().getName(), message);
	}
	...
	public void enableSlow(long seconds) {
		...
		Map<ChannelStateEvent.ChannelState, Object> states = getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getChannelState();
	...
	public void disableSlow() {
		if (!isBotModerator()) return;
		long seconds = (Long) getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getChannelState().get(ChannelStateEvent.ChannelState.SLOW);
	...
	public void enableR9K() {
		if (!isBotModerator()) return;
		boolean enabled = (Boolean) getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getChannelState().get(ChannelStateEvent.ChannelState.R9K);
	...
	public void enableSubscribers() {
		if (!isBotModerator()) return;
		boolean enabled = (Boolean) getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getChannelState().get(ChannelStateEvent.ChannelState.SUBSCRIBERS);
	...
	public void enableEmoteOnly() {
		if (!isBotModerator()) return;
		boolean enabled = (Boolean) getTwitchClient().getMessageInterface().getTwitchChat().getChannelCache().get(channel.getName()).getChannelState().get(ChannelStateEvent.ChannelState.EMOTE);
