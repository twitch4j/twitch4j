...
public class IRCEventListener {
	...
	public void onChannelMessage(IRCMessageEvent event) {
		if (event.getCommandType().equals("PRIVMSG")) {
			if (!event.getTags().containsKey("bits") && event.getMessage().isPresent()) {
				// Load Info
				Channel channel = event.getTwitchClient().getChannelEndpoint(event.getChannelId()).getChannel();
				Optional<User> user = event.getTwitchClient().getUserEndpoint().getUser(event.getUserId());

				if (user.isPresent()) {
					// Dispatch Event
					if (event.getMessage().get().startsWith("\u0001ACTION ")) {
						// Action
						getTwitchClient().getEventManager().dispatchEvent(new ChannelMessageActionEvent(channel, user.get(), event.getMessage().get().substring(8), event.getClientPermissions()));
					} else {
						// Regular Message
						getTwitchClient().getEventManager().dispatchEvent(new ChannelMessageEvent(channel, user.get(), event.getMessage().get(), event.getClientPermissions()));
					}
				}
			}
		}
	...
	public void onWhisper(IRCMessageEvent event) {
		if (event.getCommandType().equals("WHISPER")) {
			// Load Info
			User user = event.getTwitchClient().getUserEndpoint().getUser(event.getUserId()).get();

			// Dispatch Event
			getTwitchClient().getEventManager().dispatchEvent(new PrivateMessageEvent(user, event.getMessage().get(), event.getClientPermissions()));
		}
	}
	...
	public void onChannelCheer(IRCMessageEvent event) {
		if (event.getCommandType().equals("PRIVMSG")) {
			if (event.getTags().containsKey("bits")) {
				...
				// Dispatch Event
				getTwitchClient().getEventManager().dispatchEvent(new CheerEvent(channel, user, message, bits));
			}
		}
	}
	...
	public void onChannelSubscription(IRCMessageEvent event) {
		if (event.getCommandType().equals("USERNOTICE") && event.getTags().containsKey("msg-id")) {
			if (event.getTags().get("msg-id").equalsIgnoreCase("sub") || event.getTags().get("msg-id").equalsIgnoreCase("resub")) {
				// Load Info
				Channel channel = event.getTwitchClient().getChannelEndpoint(event.getChannelName().get()).getChannel();
				User user = event.getTwitchClient().getUserEndpoint().getUserByUserName(event.getTagValue("login").get()).get();
				...
				// Dispatch Event
				getTwitchClient().getEventManager().dispatchEvent(new SubscriptionEvent(channel, entity));
			}
		}
	}
		...
		if (event.getCommandType().equals("CLEARCHAT")) {
			...
			if (event.getTags().containsKey("target-user-id")) { // ban or timeout
				if (event.getTags().containsKey("ban-duration")) { // timeout
					// Load Info
					User user = event.getTwitchClient().getUserEndpoint().getUser(Long.parseLong(event.getTags().get("target-user-id"))).get();
					...
					// Dispatch Event
					getTwitchClient().getEventManager().dispatchEvent(timeoutEvent);
				} else { // ban
					// Load Info
					User user = event.getTwitchClient().getUserEndpoint().getUser(Long.parseLong(event.getTagValue("target-user-id").get())).get();
					...
					// Dispatch Event
					getTwitchClient().getEventManager().dispatchEvent(banEvent);
				}
			} else { // Clear chat event
				getTwitchClient().getEventManager().dispatchEvent(new ClearChatEvent(channel));
			}
		}
	}
	...
	public void onChannnelClientJoinEvent(IRCMessageEvent event) {
		if (event.getCommandType().equals("JOIN") && event.getChannelName().isPresent() && event.getClientName().isPresent()) {
			// Load Info
			Channel channel = event.getTwitchClient().getChannelEndpoint(event.getChannelName().get()).getChannel();
			User user = event.getTwitchClient().getUserEndpoint().getUserByUserName(event.getClientName().get()).get();

			// Dispatch Event
			getTwitchClient().getEventManager().dispatchEvent(new ChannelJoinEvent(channel, user));
		}
	}
	...
	public void onChannnelClientLeaveEvent(IRCMessageEvent event) {
		if (event.getCommandType().equals("PART") && event.getChannelName().isPresent() && event.getClientName().isPresent()) {
			...
			// Dispatch Event
			getTwitchClient().getEventManager().dispatchEvent(new ChannelLeaveEvent(channel, user));
		}
	}
	...
	public void onChannelModChange(IRCMessageEvent event) {
		if (event.getCommandType().equals("MODE") && event.getPayload().isPresent()) {
			...
			if (event.getPayload().get().substring(1).startsWith("o")) {
				// Load Info
				Channel channel = event.getTwitchClient().getChannelEndpoint(event.getChannelName().get()).getChannel();
				User user = event.getTwitchClient().getUserEndpoint().getUserByUserName(event.getPayload().get().substring(3)).get();

				// Dispatch Event
				getTwitchClient().getEventManager().dispatchEvent(new ChannelModEvent(channel, user, event.getPayload().get().startsWith("+")));
			}
		}
	}
	...
	public void onNoticeEvent(IRCMessageEvent event) {
		if (event.getCommandType().equals("NOTICE")) {
			...
			getTwitchClient().getEventManager().dispatchEvent(new ChannelNoticeEvent(channel, messageId, message));
		}
	}
	...
	public void onHostOnEvent(IRCMessageEvent event) {
		if (event.getCommandType().equals("NOTICE")) {
			...
			if (messageId.equals("host_on")) {
				...
				Channel targetChannel = event.getTwitchClient().getChannelEndpoint(targetChannelName).getChannel();

				getTwitchClient().getEventManager().dispatchEvent(new HostOnEvent(channel, targetChannel));
			}
		}
	}
	...
	public void onHostOffEvent(IRCMessageEvent event) {
		if (event.getCommandType().equals("NOTICE")) {
			...
			if (messageId.equals("host_off")) {
				getTwitchClient().getEventManager().dispatchEvent(new HostOffEvent(channel));
			}
		}
	}
	...
	public void onChannelState(IRCMessageEvent event) {
		if (event.getCommandType().equals("ROOMSTATE")) {
			...
			getTwitchClient().getEventManager().dispatchEvent(new ChannelStateEvent(channel, states));
		}
	}
}
