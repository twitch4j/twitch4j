	...
	public void onRawIrcMessage(IrcRawMessageEvent event) {
		if(event.getIrcParser().getCommand().equals("PRIVMSG")) {
			if(!event.getIrcParser().getTags().hasTag("bits")) {
				...
				// Dispatch Event
				ChannelMessageActionEvent channelMessageActionEvent = new ChannelMessageActionEvent(channel, user, event.getIrcParser().getMessage(), event.getIrcParser().getPermissions());
				event.getClient().getDispatcher().dispatch(channelMessageActionEvent);
			}
		}
	}
